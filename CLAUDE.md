...
- Load `common/language-korean.md` to write all generated output in Korean
- Load `common/nondev-question-style.md` to make every question non-developer friendly (plain explanations + a recommended default)
- Load `common/runtime-bootstrap.md` to auto-install any missing runtime (Python/Node) during build/test instead of asking the user
...
## Inception Wrap-up Mockup (ALWAYS EXECUTE — end of INCEPTION)

Before transitioning to CONSTRUCTION, you MUST follow `common/visual-mockup-guide.md` →
"ALWAYS: inception wrap-up mockup": generate at least one mockup of the product's main screen(s)
and/or primary user flow based on everything decided in inception, save it to
`aidlc-docs/inception/mockups/inception-summary.html`, and ask the user (in Korean) to open it in a
browser and confirm it matches their expectation. Do this even if no earlier mockup trigger fired.
